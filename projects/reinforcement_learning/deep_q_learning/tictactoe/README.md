---
layout: post
title: "Tic-Tac-Toe Deep Q-Network Application"
date: 2026-02-09
...
---
## Overview
Deep Q-Network (DQN) is a reinforcement learning algorithm that trains an agent to associate specific actions with long-term rewards. Instead of maintaining an explicit Q-Table (as in Q-Learning), a DQN utilizes a Deep Neural Network as a function approximator to estimate Q-Values for any given state.

By using the Bellman Equation as a loss function, the network is iteratively-trained to minimize the difference between its current predictions and the target optimal rewards.

## Problem Statement
Use DQN to train an unbeatable Tic-Tac-Toe agent. 
